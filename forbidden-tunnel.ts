...
export function simulateForbiddenTunnels(iterations:number,seed=20260823){let s=seed|0;const rng=()=>{s=s+0x6D2B79F5|0;let t=Math.imul(s^s>>>15,1|s);t=t+Math.imul(t^t>>>7,61|t)^t;return((t^t>>>14)>>>0)/4294967296};const first={x1:0,x2:0,x5:0},second={sealed:0,deep:0};let chance=0;for(let i=0;i<iterations;i++){const state=createForbiddenTunnel(rng,String(i));let result=selectFirstPath(state,FIRST_DIRECTIONS[Math.floor(rng()*3)],rng);const firstOutcome=state.firstAssignments[result.tunnel.firstSelection!];first[firstOutcome]++;if(firstOutcome==="x5"){result=selectSecondPath(result.tunnel,SECOND_DIRECTIONS[Math.floor(rng()*2)]);second[result.tunnel.resolution as SecondOutcome]++}chance+=result.modifier!.chance}const average=chance/iterations;return {iterations,first,second,averageTunnelChance:average,overallBaselineRate:.99*ORDINARY_TRUE_ARTIFACT_CHANCE+.01*average};}
