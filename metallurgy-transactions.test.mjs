...
test("UI separates lifetime history from stock, confirms transactions, and preserves cosmetics",()=>{
  ...
  assert.match(game,/EFFECTIVE TRUE ARTEFACT CHANCE/);assert.match(game,/artifactChanceForDig\(save\.pendingArtifactModifier,current\.trueArtifactChance\)/);
...
test("migration is schema-versioned and idempotent by construction",()=>{
  ...
  assert.match(game,/toolTier=Math\.max/);assert.match(game,/schema:18/);
});
