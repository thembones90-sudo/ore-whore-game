...
import { CANONICAL_EXCAVATION_PROBABILITIES, canAfford, forgeAtomic, forgeRecipes, forgedItems, metallurgyRecipes, processedMaterials, smeltAtomic, type RecipeUnlock, type ResourceState } from "./metallurgy";
...
const migrate=(old:any):Save=>{
  ...
  const provisional={...blank,...safeOld,combos,ores:oreCounts,oreResources,mineralResources,processedResources,ownedTools,equippedTool,toolTier,toolSkinId,first,minerals:mineralCounts,activeTrueEncounter:validEncounter,forbiddenTunnel,pendingArtifactModifier,settings:{...defaultSettings,...old.settings},schema:15} as Save;
...
export default function Home() {
  ...
  const strike = (point?:{x:number;y:number}) => {
    ...
    if (stage === "tunnel") {
      ...
      const modifier=save.pendingArtifactModifier,trueChance=artifactChanceForDig(modifier),artifactWon=rng.current()<trueChance;
...
function TrueArchive({save}:{save:Save}){
  ...
  return <><section className="page true-archive-page">
    ...
    <p className="true-archive-intro">Common through Legendary belongs to the mountain. These do not. Every ordinary completed dig has a <strong>0.05% TRUE Artifact chance</strong> · 1 in 2,000. Tool technology does not modify this probability. Only a Forbidden Tunnel can temporarily alter the next dig&apos;s chance. No pity system. No stacking.{totalFound?` Total anomalies logged: ${totalFound}.`:""}</p>
