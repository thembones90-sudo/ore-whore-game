...
type Save = { digs: number; strikes: number; distance: number; combos: Record<string, number>; ores: Record<string, number>; minerals: Record<string, number>; first: Record<string, number>; achievements: string[]; streak: number; longestStreak:number;newStreak:number;longestNewStreak:number; dust: number; dustEarned:number;dustSpent:number; biome: Biome; milestones: Record<string, number>; lastDigAt: number; schema: number; settings:Settings;unlocks:string[];equipped:string;huntTarget:string|null;huntCounts:Record<string,number>;huntStartedAtDig:number;longestHunt:number };
...
const blank: Save = { digs: 0, strikes: 0, distance: 0, combos: {}, ores: {}, minerals: {}, first: {}, achievements: [], streak: 0,longestStreak:0,newStreak:0,longestNewStreak:0, dust: 0,dustEarned:0,dustSpent:0, biome: "old", milestones: {}, lastDigAt: 0, schema: 4,settings:defaultSettings,unlocks:[],equipped:"standard",huntTarget:null,huntCounts:{},huntStartedAtDig:0,longestHunt:0 };
...
function Wanted({save,onHunt}:{save:Save;onHunt:(b:Biome,ore:string)=>void}){
  ...
  const shown=rows.map(r=>({...r,missing:r.missing.filter(m=>(!query||`${r.ore.name} ${m.name}`.toLowerCase().includes(query.toLowerCase()))&&(!rareOnly||["Legendary","Mythic"].includes(m.rarity))&&(!currentOnly||save.huntTarget===`${r.ore.id}-${m.id}`))})).filter(r=>r.missing.length&&(mine==="all"||bestBiome(r.ore)===mine));
