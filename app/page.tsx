...
export default function Home() {
  ...
  const strike = (point?:{x:number;y:number}) => {
    ...
    if (stage === "tunnel") {
      ...
      const modifier=save.pendingArtifactModifier,asocChance=asocTicketChanceForDig(save.equippedTool,save.trueArtifacts),asocWon=asocChance>0&&rng.current()<asocChance,trueChance=artifactChanceForDig(modifier,equippedMiningTool(save).trueArtifactChance),artifactWon=!asocWon&&rng.current()<trueChance;
...
function TrueArchive({save}:{save:Save}){
  ...
  const equipped=equippedMiningTool(save),effectiveChance=artifactChanceForDig(save.pendingArtifactModifier,equipped.trueArtifactChance),asocChance=asocTicketChanceForDig(save.equippedTool,save.trueArtifacts);
  return <><section className="page true-archive-page">
    ...
    <p className="true-archive-intro">Common through Legendary belongs to the mountain. These do not. Equipped tool: <strong>{equipped.name}</strong>. Current ordinary TRUE Artifact chance: <strong>{(effectiveChance*100).toFixed(2)}%</strong> · approximately 1 in {Math.round(1/effectiveChance).toLocaleString()}. Forbidden Tunnel modifiers apply only to the ordinary roll on the next completed dig. Golden ASOC Ticket: <strong>{save.trueArtifacts[ASOC_TICKET_ID]?"ISSUED — UNIQUE":asocChance?`${(ASOC_TICKET_CHANCE*100).toFixed(1)}% · 1 IN ${Math.round(1/ASOC_TICKET_CHANCE).toLocaleString()}`:"LOCKED — REQUIRES MOUNTAIN FUCKER"}</strong>. Its roll is separate and never stacks with tunnel modifiers. No pity system.{totalFound?` Total anomalies logged: ${totalFound}.`:""}</p>
