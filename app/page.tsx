...
export default function Home() {
  ...
  return <main className={`tab-${tab} ${save.settings.reducedMotion?"reduced-motion":""} ${save.settings.reducedShake?"reduced-shake":""} ${save.settings.highContrast?"high-contrast":""} cosmetic-${save.equipped} ${currentBerserk?`berserk-active berserk-${currentBerserk.id}`:""}`} style={{"--berserk-speed":currentBerserk?.intervalMultiplier||1,"--berserk-power":currentBerserk?.damageMultiplier||1} as React.CSSProperties}>
    ...
    {save.forbiddenTunnel&&<ForbiddenTunnelEncounter state={save.forbiddenTunnel} contractCallback={!save.forbiddenContractSeen} reducedMotion={save.settings.reducedMotion} onFirst={selectForbiddenFirst} onSecond={selectForbiddenSecond} onContinue={()=>{setSave(s=>({...s,forbiddenTunnel:null}));setTab("mine");continueMine()}}/>}
...
function ForbiddenTunnelEncounter({state,contractCallback,reducedMotion,onFirst,onSecond,onContinue}:{state:ForbiddenTunnelState;contractCallback:boolean;reducedMotion:boolean;onFirst:(direction:FirstDirection)=>void;onSecond:(direction:SecondDirection)=>void;onContinue:()=>void}){
...
function Onboarding({agreementRequired,onDone,existingName}:{agreementRequired:boolean;onDone:(signed:boolean,playerName?:string)=>void;existingName:string}){
  const [phase,setPhase]=useState<"title"|"management"|"contract"|"signing">("title"),[declined,setDeclined]=useState(false),[employeeName,setEmployeeName]=useState(existingName),[signedName,setSignedName]=useState("");
  ...
  useEffect(()=>{if(phase!=="signing")return;const timer=setTimeout(()=>onDoneRef.current(true,signedName),2200);return()=>clearTimeout(timer)},[phase,signedName]);
  if(phase==="signing")return <div className="onboarding employment-signing shadez-signing"><section><ShadezAvatar/><small>FOREMAN SHADEZ</small><h2>Agreement accepted.</h2><em className="contract-signature">SIGNED: {signedName}</em><p>Welcome to ORE WHORE, <strong>{signedName}</strong>.</p><p>Your employment has commenced.</p><div className="signing-exchange"><b>PEON</b> Me have job now?<br/><b>SHADEZ</b> Regrettably.</div><span>ASSIGNMENT · OLD MINE</span></section></div>;
  if(phase==="management")return <div className="onboarding shadez-recruitment"><section><ShadezAvatar/><div><small>A MESSAGE FROM MANAGEMENT</small><h2>FOREMAN SHADEZ</h2><p>Children yearn for the mines.</p><p>Unfortunately, we are out of children.</p><strong>So you will do.</strong><button onClick={()=>setPhase("contract")}>PRESENT AGREEMENT →</button></div></section></div>;
  ...
  return <div className="onboarding"><div><img className="onboarding-logo" src="/assets/brand/ore-whore-logo-primary.webp" alt="ORE WHORE — Property of the Department"/><p className="eyebrow">VOLUME I · CLASSIC → TBC → WOTLK</p><h1>DIG. CLANK. CRACK. <i>COLLECT.</i></h1><p>Fifteen ores. Fifteen minerals. 225 reasons not to stop.</p><button onClick={()=>agreementRequired?setPhase("management"):onDone(false,existingName)}>{agreementRequired?"REPORT TO MANAGEMENT":"RETURN TO MINING"} <span>→</span></button><small>{agreementRequired?"One vacancy. No meaningful qualifications required.":"Click the rock or press Space. That is genuinely it."}</small></div></div>;
}
