...
type Save = { digs: number; emptyDigs:number; strikes: number; distance: number; combos: Record<string, number>; ores: Record<string, number>; minerals: Record<string, number>; first: Record<string, number>; achievements: string[]; streak: number; longestStreak:number;newStreak:number;longestNewStreak:number; dust: number; dustEarned:number;dustSpent:number; biome: Biome; unlockedBiomes:Biome[]; completedBiomes:Biome[]; milestones: Record<string, number>; lastDigAt: number; schema: number; settings:Settings;unlocks:string[];equipped:string;huntTarget:string|null;huntCounts:Record<string,number>;huntStartedAtDig:number;longestHunt:number };
...
export default function Home() {
  ...
  const strike = (point?:{x:number;y:number}) => {
    ...
    if (stage === "tunnel") {
      if(rng.current()<.2){playImpact("crack");setSave(s=>{const streak=s.streak+1;return {...s,digs:s.digs+1,emptyDigs:s.emptyDigs+1,streak,longestStreak:Math.max(s.longestStreak,streak),newStreak:0,lastDigAt:Date.now()}});sessionDigs.current++;setEmptyFind(true);track("dig_empty",{attempt:save.digs+1,biome:save.biome,empty_rate:.2});return;}
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      if (event.code === "Space" && tab === "mine" && !found && !emptyFind) { event.preventDefault(); strike(); }
    };
  ...
  return <main className={`${save.settings.reducedMotion?"reduced-motion":""} ${save.settings.reducedShake?"reduced-shake":""} ${save.settings.highContrast?"high-contrast":""} cosmetic-${save.equipped}`}>
    ...
    {tab === "mine" && <section className={`mine-screen biome-${save.biome} ${impact ? "screen-hit" : ""} stage-${stage}`}>
      ...
      <button className={`rock ${impact ? "hit" : ""} ${stage === "ore" ? "ore-rock" : ""} damage-${Math.floor((1-rockHp/maxHp)*4)} ${rockHp===1?"final-hit":""}`} style={{"--hit-x":`${hitPoint.x}%`,"--hit-y":`${hitPoint.y}%`} as React.CSSProperties} onClick={strikeAtPointer} aria-label={stage === "ore" ? "Crack the exposed ore deposit" : "Strike the rock wall"}>
        ...
        {stage === "ore" && pendingOre && <span className="exposed-ore" style={{"--ore":pendingOre.color} as React.CSSProperties}><i className="ore-cluster" aria-hidden="true"><b/><b/><b/><b/><b/></i><strong>{pendingOre.name}</strong><small>{pendingOre.rarity.toUpperCase()}</small></span>}
...
function Album({ save }: { save: Save }) {
  ...
  return <section className="page album-page"><div className="page-head"><div><p className="eyebrow">CLASSIC → TBC → WOTLK</p><h2>VOLUME I <i>ALBUM</i></h2></div><div className="completion"><span>VOLUME COMPLETION</span><strong>{unique} <small>/ 225 · {(unique/225*100).toFixed(1)}%</small></strong></div></div><div className="volume-overview"><span><small>ORES</small>{oreFound}/15</span><span><small>MINERALS</small>{mineralFound}/15</span><span><small>COMPLETED PAGES</small>{completed}/15</span><span><small>MISSING</small>{225-unique}</span></div><div className="overview-highlights"><span><small>RAREST DISCOVERY</small>{globalRare?`${globalRare.o.name} + ${globalRare.m.name} · 1/${globalRare.odd}`:"Nothing worth mentioning yet."}</span><span><small>MOST WANTED</small>{wanted}</span></div>
    ...
    <div className={`album-title milestone-${total}`}><div><span className="ore-gem gem-art" style={{"--gem":selected.color} as React.CSSProperties}/><div><p>{selected.rarity.toUpperCase()} ORE · {save.ores[selected.id]||0} MINED</p><h3>{selected.name} {total===15&&<b className="complete-stamp">PAGE COMPLETE</b>}</h3><small>{total===14?`ONE SPECIMEN REMAINS: ${minerals.find(m=>!save.combos[`${selected.id}-${m.id}`])?.name}. WHERE IS IT?`:selected.note}</small><div className="page-stats">RAREST: {rarest?.m.name||"—"} · MOST DUPLICATED: {most?`${most.m.name} ×${most.count}`:"—"} · MISSING: {15-total}</div></div></div><strong>{total}<small>/15 · {Math.round(total/15*100)}%</small></strong></div>
...
function ShareCard({data,biome,onClose}:{data:{ore:Item;mineral:Item;attempt:number;total:number};biome:Biome;onClose:()=>void}){return <div className="share-wrap"><div className="share-card"><p>ORE WHORE · VOLUME I</p><div><span className="gem-art" style={{"--gem":data.ore.color} as React.CSSProperties}/><b>+</b><span className="gem-art" style={{"--gem":data.mineral.color} as React.CSSProperties}/></div><small>{data.ore.rarity.toUpperCase()} + {data.mineral.rarity.toUpperCase()}</small><h2>{data.ore.name}<br/>+ {data.mineral.name}</h2><strong>RARE DISCOVERY</strong><footer><span>NATURAL ODDS · 1 / {odds(data.ore,data.mineral,biome)}</span><span>ATTEMPT #{data.attempt} · ALBUM {data.total}/225</span></footer></div><p>Screenshot this. Nobody will believe you, but try.</p><button onClick={onClose}>CONTINUE</button></div>}
...
function Reveal({ found, total, biome, onContinue }: { found: {ore:Item;mineral:Item;isNew:boolean;count:number}; total:number;biome:Biome; onContinue:()=>void }) {
  ...
  return <div className={`reveal ore-${found.ore.id} ${huge?"mythic":""} ${!found.isNew&&!huge?"quick":""}`}><div className="reveal-card"><button className="close" onClick={onContinue}>×</button><p className="eyebrow">{personality}</p><div className="combo-art"><span className="big-gem ore gem-art" style={{"--gem":found.ore.color} as React.CSSProperties}/><b>+</b><span className="big-gem mineral gem-art" style={{"--gem":found.mineral.color} as React.CSSProperties}/></div><div className="names"><div><small>{found.ore.rarity}</small><strong>{found.ore.name}</strong></div><b>CONTAINING</b><div><small>{found.mineral.rarity}</small><strong>{found.mineral.name}</strong></div></div><div className={`verdict ${found.isNew?"new":"duplicate"}`}><span>{found.isNew ? (huge?"MYTHIC DISCOVERY":"NEW COMBINATION") : `DUPLICATE ×${found.count}`}</span><strong>{found.isNew ? `${total} / 225` : `+${dustByRarity[found.mineral.rarity]} SPECIMEN DUST`}</strong><small>NATURAL ODDS · APPROX. 1 IN {comboOdds.toLocaleString()}</small></div><button className="continue" onClick={onContinue}>{found.isNew?"CONTINUE MINING":"AGAIN. NOW."} <span>→</span></button></div></div>;
}
