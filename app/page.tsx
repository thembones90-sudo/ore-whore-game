...
const mineDepletionLevel=(save:Save,biome:Biome)=>{const pct=biomeQuotaProgress(save,biome)/biomeQuotaTotal(biome);if(biome==="old")return pct>=.9?4:pct>=.65?3:pct>=.4?2:pct>=.2?1:0;return pct>=1?4:pct>=.75?3:pct>=.5?2:pct>=.25?1:0};
  ...
  return <main className={`tab-${tab} ${save.settings.reducedMotion?"reduced-motion":""} ${save.settings.reducedShake?"reduced-shake":""} ${save.settings.highContrast?"high-contrast":""} cosmetic-${save.equipped}`}>
    ...
    {tab === "mine" && <section className={`mine-screen biome-${save.biome} ${impact ? "screen-hit" : ""} stage-${stage}`} style={{"--biome-accent":biomeVisuals[save.biome].accent,"--biome-secondary":biomeVisuals[save.biome].secondary,"--biome-canvas":biomeVisuals[save.biome].canvas,"--biome-cavity":biomeVisuals[save.biome].cavity,"--biome-debris":biomeVisuals[save.biome].debris,"--biome-particle":biomeVisuals[save.biome].particle,"--biome-light":biomeVisuals[save.biome].light} as React.CSSProperties}>
      ...
      <button className={`rock depletion-${mineDepletionLevel(save,save.biome)} motion-${activeSkin.animation.id} ${miningEngaged&&activeSkin.animation.engagedLoop?"mining-engaged":""} ${impact ? "hit" : ""} ${stage === "ore" ? "ore-rock" : ""} ${stage === "artifact" ? "artifact-rock" : ""} damage-${Math.floor((1-rockHp/maxHp)*4)} ${rockHp===1?"final-hit":""} ${lastHitKind?`hit-${lastHitKind==="perfectCrit"?"perfect-crit":lastHitKind}`:""}`} style={{"--hit-x":`${hitPoint.x}%`,"--hit-y":`${hitPoint.y}%`} as React.CSSProperties} onPointerEnter={followPointer} onPointerMove={followPointer} onPointerLeave={stopFollowing} onClick={strikeAtPointer} aria-label={stage === "artifact" ? "Excavate the unidentified impossible object" : stage === "ore" ? "Crack the exposed ore deposit" : "Strike the rock wall"}>
        ...
        {save.biome==="old"&&<span className="old-mine-stage-art" aria-hidden="true">{Array.from({length:5},(_,i)=><i key={i} className={`old-stage-${i}`}/>)}</span>}
