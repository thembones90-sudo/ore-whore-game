...
const biomeNames:Record<Biome,string>={old:"OLD MINE",deep:"DEEP MINE",outland:"OUTLAND MINE",northrend:"NORTHREND MINE",ghost:"GHOST MINES",moon:"MOON BASE"};
const biomeVisuals:Record<Biome,{accent:string;secondary:string;canvas:string;cavity:string;debris:string;particle:string;card:string;texture:string;light:string;flavor:string}>={
 ...
 ghost:{accent:"#6b7570",secondary:"#3c4440",canvas:"#050604",cavity:"#020302",debris:"#3a3f3b",particle:"#5a625c",card:"#0a0c0a",texture:"damp",light:"#8a9a92",flavor:"Nobody built this. Nobody explains it."},
...
function Album({ save }: { save: Save }) {
  ...
  return <section className="page album-page"><div className="page-head"><div><p className="eyebrow">CLASSIC → TBC → WOTLK</p><h2>VOLUME I <i>ALBUM</i></h2></div><div className="completion"><span>VOLUME COMPLETION</span><strong>{unique} <small>/ 225 · {(unique/225*100).toFixed(1)}%</small></strong></div></div><div className="volume-overview"><span><small>ORES</small>{oreFound}/15</span><span><small>MINERALS</small>{mineralFound}/15</span><span><small>COMPLETED PAGES</small>{completed}/15</span><span><small>MISSING</small>{225-unique}</span></div><div className="overview-highlights"><span><small>RAREST DISCOVERY</small>{globalRare?`${globalRare.o.name} + ${globalRare.m.name} · 1/${globalRare.odd}`:"Nothing worth mentioning yet."}</span><span><small>MOST WANTED</small>{wanted}</span></div>
    <div className="ore-tabs volume-tabs">{ores.map(o => {const n=minerals.filter(m=>save.combos[`${o.id}-${m.id}`]).length,concealed=ghostOreConcealed(save,o.id);return <button key={o.id} className={`${selected.id === o.id ? "selected" : ""} ${n===15?"complete":""}`} onClick={() => {setSelected(o);track("ore_page_opened",{ore_id:o.id,completion:n/15})}}>{concealed?<div className="ore-tab-art gem-art" style={{"--gem":"#2b2d2e"} as React.CSSProperties}/>:<img className="ore-tab-art" src={oreAsset(o.id)} alt=""/>}<span>{concealed?"UNKNOWN":o.name.replace(" Ore","")}<small>{n}/15 {n===15?"✓":""}</small></span></button>})}</div>
    <div className={`album-title milestone-${total} ${total===14?`last-one-${lastOneTier(save,selected.id)}`:""}`}>{ghostOreConcealed(save,selected.id)?<div><div className="ore-gem ore-sprite ore-sprite-album gem-art" style={{"--gem":"#2b2d2e"} as React.CSSProperties}/><div><p>UNKNOWN ORE</p><h3>UNDISCOVERED</h3><small>Its identity has not been established.</small><div className="page-stats">RAREST: — · MOST DUPLICATED: — · MISSING: 15</div></div></div>:<div><img className="ore-gem ore-sprite ore-sprite-album" src={oreAsset(selected.id)} alt=""/><div><p>{selected.rarity.toUpperCase()} ORE · {save.ores[selected.id]||0} MINED</p><h3>{selected.name} {total===15&&<b className="complete-stamp">PAGE COMPLETE</b>}</h3><small>{total===14?`${lastOneCopy(save,selected.id)} MISSING: ${minerals.find(m=>!save.combos[`${selected.id}-${m.id}`])?.name}.`:selected.note}</small><div className="page-stats">RAREST: {rarest?.m.name||"—"} · MOST DUPLICATED: {most?`${most.m.name} ×${most.count}`:"—"} · MISSING: {15-total}</div></div></div>}<strong>{total}<small>/15 · {Math.round(total/15*100)}%</small></strong></div>
...
function MineCompletion({data,digs,playerName,onContinue}:{data:{completed:Biome;next?:Biome};digs:number;playerName:string;onContinue:()=>void}){
 const [descending,setDescending]=useState(false);
 const destination=data.next||data.completed,v=biomeVisuals[destination],completedShaft=String(biomeOrder.indexOf(data.completed)+1).padStart(2,"0"),shaft=String(biomeOrder.indexOf(destination)+1).padStart(2,"0"),contractTransfer=data.completed==="old"&&data.next==="deep",beyondMaps=data.completed==="northrend"&&data.next==="ghost";
 const diagnostics:Record<Biome,[string,string,string]>={old:["DEPTH: SHALLOW","LIGHT: INADEQUATE","RESCUE COVERAGE: THEORETICAL"],deep:["DEPTH: INADVISABLE","LIGHT: INSUFFICIENT","RESCUE COVERAGE: NONE"],outland:["AIR: LEGALLY QUESTIONABLE","CORRUPTION: ACTIVE","MEDICAL COVERAGE: DENIED"],northrend:["TEMPERATURE: HOSTILE","VISIBILITY: DECLINING","RECOVERY WINDOW: CLOSED"],ghost:["ORIGIN: UNKNOWN","BIOLOGICAL ACTIVITY: NONE DETECTED","SURVEY MATCH: NONE"],moon:["ATMOSPHERE: ABSENT","RETURN VECTOR: UNDEFINED","RESCUE COVERAGE: NOT APPLICABLE"]};
...
function PeonAvatar(){return <img className="peon-voice-avatar" src="/assets/characters/peon-avatar.png" alt="Peon"/>}
function ShadezAvatar(){return <img className="shadez-voice-avatar" src="/assets/characters/foreman-shadez.png" alt="Foreman Shadez"/>}
...
function GhostEntry({reducedMotion,onContinue}:{reducedMotion:boolean;onContinue:()=>void}){
  ...
  return <div className={`ghost-entry stage-${stage}`} role="status" aria-live="assertive">
    <div className="ghost-entry-card">
      {stage==="opening" && <div className="ghost-entry-system"><small>SYSTEM</small>{GHOST_ENTRY_SYSTEM_OPENING.slice(0,visibleLines).map((line,index)=><p key={line} className={index===visibleLines-1?"current":"spoken"}>{line}</p>)}</div>}
      ...
      {stage==="closing" && <div className="ghost-entry-closing">{GHOST_ENTRY_CLOSING.map(line=><p key={line.text} className={line.speaker==="PEON"?"peon-voiced":line.speaker==="SHADEZ"?"shadez-voiced":""}>{line.speaker==="PEON"&&<PeonAvatar/>}{line.speaker==="SHADEZ"&&<ShadezAvatar/>}<small>{line.speaker}</small>{line.text}</p>)}
...
function TrueArchive({save}:{save:Save}){
  ...
  const equipped=equippedMiningTool(save),effectiveChance=artifactChanceForDig(save.pendingArtifactModifier,equipped.trueArtifactChance),asocKnown=save.asocTickets>0;
  ...
  return <><section className="page true-archive-page">
    ...
    <p className="true-archive-intro">Common through Legendary belongs to the mountain. These do not. Equipped tool: <strong>{equipped.name}</strong>. Current ordinary TRUE Artifact chance: <strong>{(effectiveChance*100).toFixed(2)}%</strong> · approximately 1 in {Math.round(1/effectiveChance).toLocaleString()}. Forbidden Tunnel modifiers apply only to the ordinary roll on the next completed dig. {asocKnown?<>Golden ASOC Tickets secured: <strong>{save.asocTickets}</strong>. Current run: <strong>{save.gameCompleted?"SHIFT COMPLETE":"INVITATION SECURED"}</strong>.</>:<>Endgame condition: <strong>CLASSIFIED</strong>. Current run: <strong>CLEARANCE INSUFFICIENT</strong>.</>} No pity system.{totalFound?` Total anomalies logged: ${totalFound}.`:""}</p>
...
const RECRUITMENT_DIALOGUE:AvatarDialogueLine[]=[
  {speaker:"PEON",text:"Boss?"},{speaker:"SHADEZ",text:"You are here to work."},{speaker:"PEON",text:"Me good at work."},{speaker:"SHADEZ",text:"That remains to be seen."},
...
const PAPERWORK_DIALOGUE:AvatarDialogueLine[]=[
  {speaker:"SHADEZ",text:"There is, regrettably, paperwork."},{speaker:"PEON",text:"Me hit paperwork?"},{speaker:"SHADEZ",text:"No."},{speaker:"PEON",text:"Bad job."},
...
function Onboarding({agreementRequired,onDone,existingName}:{agreementRequired:boolean;onDone:(signed:boolean,playerName?:string)=>void;existingName:string}){
  ...
  if(phase==="signing")return <div className="onboarding dialogue-onboarding signed-contract-stage"><div className="signed-contract-backdrop"><small>INDEFINITE EXCAVATION AGREEMENT</small><em className="contract-signature">SIGNED: {signedName}</em></div><AvatarDialogue ariaLabel="Signed employment dialogue" lines={[{speaker:"SHADEZ",text:`Agreement accepted.\n\nWelcome to ORE WHORE, ${signedName}.\n\nYour employment has commenced.`},{speaker:"PEON",text:"Me have job now?"},{speaker:"SHADEZ",text:"Regrettably."}]} onComplete={()=>onDone(true,signedName)}/></div>;
...
function Forge({save,setSave}:{save:Save;setSave:React.Dispatch<React.SetStateAction<Save>>}){
  ...
  const RecipeCard=({id}:{id:string})=>{const r=metallurgyRecipes.find(x=>x.id===id)!,open=recipeUnlocked(save,r.unlock),ready=open&&canAfford(save.oreResources,r.inputs),owned=save.processedResources[r.outputId]||0,needed=nextToolRecipe?.processedInputs.find(i=>i.id===r.outputId)?.quantity||0,short=Math.max(0,needed-owned);const act=()=>{track("recipe_viewed",{recipe_id:r.id,ready,missing:missing(save.oreResources,r.inputs)});if(!open)return notify("error","MACHINE LOCKED",`Reach the ${r.unlock?.mine?.toUpperCase()} MINE to use this station.`);if(!ready)return notify("error","NOT ENOUGH ROCK",`Missing: ${missing(save.oreResources,r.inputs)}.`);runMetallurgy(r.id)};return <article className={`machine-recipe ${!open?"locked":""}`}><header><span>{r.operation}</span><strong>{r.name}</strong></header><div className="recipe-equation">{r.inputs.map(i=><b key={i.id}>{craftingName(i.id)} ×{i.quantity}</b>)}<i>→</i><em>{craftingName(r.outputId)} ×{r.outputQuantity}</em></div><div className="recipe-stock"><span>YOU OWN <strong>{owned}</strong></span><span>{needed?<>NEXT TOOL NEEDS <strong>{needed}</strong></>:<>NEXT TOOL <strong>—</strong></>}</span><span className={short?"missing":"complete"}>{needed?(short?<>MISSING <strong>{short}</strong></>:<>READY <strong>✓</strong></>):<>NOT REQUIRED</>}</span></div><button className={!ready?"unavailable":""} aria-disabled={!ready} onClick={act}>{open?(ready?"CREATE":`MISSING ${missing(save.oreResources,r.inputs)}`):`LOCKED · ${r.unlock?.mine?.toUpperCase()} MINE`}</button></article>};
  return <section className="forge-page"><div className="workshop-smoke" aria-hidden="true"/><div className="workshop-header"><aside className="goblin-corner" aria-hidden="true"><span className="goblin-head">⌁</span><p>PEON SMART.<br/>PEON BUILD.<br/>PEON DIG DEEP.<br/><b>ZUG ZUG!</b></p></aside><div className="workshop-sign"><span>PEON TECHNOLOGY</span><small>IF IT BONKS, IT WORKS.</small></div><div className="equipped-board equipped-loadout"><div className="loadout-technology"><span>TECHNOLOGY · GAMEPLAY</span><strong>{current.name}</strong><small>{current.technicalName}</small><ToolArt tool={current}/></div><div className="loadout-model"><span>MODEL · APPEARANCE</span><strong>{selectedSkin.name}</strong><small>{selectedSkin.technicalName}</small><img src={selectedSkin.artwork} alt=""/></div><footer><b>MODE <em>{current.inputMode.toUpperCase()}</em></b><b>ACTION <em>{current.actionDurationMs}ms</em></b><b>IMPACT <em>{current.damage.toFixed(2)}×</em></b><b>SHAFT <em>TIER {current.shaftRating}</em></b></footer></div></div>
    ...
    <section className="stock-rack"><h3>SPENDABLE ORE STOCK</h3><div className="material-ledger raw-ledger">{ores.map(o=>{const concealed=ghostOreConcealed(save,o.id);return <article key={o.id}>{concealed?<div className="ore-gem gem-art" style={{"--gem":"#2b2d2e"} as React.CSSProperties}/>:<img src={oreAsset(o.id)} alt=""/>}<span>{concealed?"UNKNOWN ORE":o.name.replace(" Ore","")}</span><strong>{save.oreResources[o.id]||0}</strong></article>})}</div></section>
    ...
    {pathRecipe&&<section className="upgrade-path"><header><div><small>SELECTED TECHNOLOGY PATH · TIER {pathTool.tier}</small><h3>{pathTool.name}</h3><p>{pathTool.technicalName}</p></div><strong className={pathReady?"ready":"blocked"}>{save.ownedTools.includes(pathTool.id)?"OWNED":pathReady?"WORKSHOP READY":pathOpen?"MATERIALS MISSING":"BLUEPRINT LOCKED"}</strong></header><div className="upgrade-path-grid"><div><span>PROCESSED PARTS</span>{pathRecipe.processedInputs.length?pathRecipe.processedInputs.map(i=>{const have=save.processedResources[i.id]||0;return <b className={have>=i.quantity?"complete":""} key={i.id}>{craftingName(i.id)} <em>{Math.min(have,i.quantity)}/{i.quantity}</em></b>}):<b className="complete">NO PROCESSED PARTS <em>✓</em></b>}</div><div><span>MINERAL / DUST COMPONENTS</span>{pathRecipe.mineralInputs.map(i=>{const have=save.mineralResources[i.id]||0;return <b className={have>=i.quantity?"complete":""} key={i.id}>{craftingName(i.id)} <em>{Math.min(have,i.quantity)}/{i.quantity}</em></b>})}{pathRecipe.dustCost?<b className={save.dust>=pathRecipe.dustCost?"complete":""}>Specimen Dust <em>{save.dust}/{pathRecipe.dustCost}</em></b>:null}</div><div><span>RAW ORE REQUIREMENTS</span>{pathPlan?(pathPlan.oreInputs.length?pathPlan.oreInputs.map(i=>{const have=save.oreResources[i.id]||0;return <b className={have>=i.quantity?"complete":""} key={i.id}>{craftingName(i.id)} <em>{have}/{i.quantity}</em></b>}):<b className="complete">NO SMELTING REQUIRED <em>✓</em></b>):<b>PREVIOUS TECHNOLOGY REQUIRED</b>}</div></div><footer><p>{pathPlan?.crafts.length?`Workshop will run ${pathPlan.crafts.map(c=>`${craftingName(c.outputId)} ×${c.quantity}`).join(" · ")} and forge the tool in one transaction.`:"Existing processed stock and direct materials will be consumed atomically."}</p><button disabled={!pathReady||save.ownedTools.includes(pathTool.id)} onClick={()=>setConfirmation({kind:"auto-forge",id:pathRecipe.id})}>{save.ownedTools.includes(pathTool.id)?"ALREADY OWNED":pathReady?"BUILD ALL PREREQUISITES + FORGE":"COLLECT MISSING MATERIALS"}</button></footer></section>}
    ...
    <section className="arsenal"><h3>FORGED MINING TOOLS <small>BIGGER = BETTER</small></h3><div className="tool-progression">{forgedItems.map(tool=>{
      ...
      const ready=!!recipe&&open&&canAfford(save.processedResources,recipe.processedInputs)&&canAfford(save.mineralResources,recipe.mineralInputs)&&canAfford(save.oreResources,recipe.oreInputs||[])&&save.dust>=(recipe.dustCost||0);
      ...
      return <article className={`${owned?"owned":""} ${save.equippedTool===tool.id?"equipped":""} ${selectedTool===tool.id?"path-selected":""} ${!open?"planned":""}`} key={tool.id}><span>TIER {tool.tier} · {tool.inputMode.toUpperCase()}</span><h3>{tool.name}</h3><h4>{tool.technicalName}</h4><ToolArt tool={tool}/><div className="tool-stats"><b>{tool.inputMode.toUpperCase()}</b><b>{tool.actionDurationMs}ms</b><b>{tool.damage.toFixed(2)}×</b><b>SHAFT {tool.shaftRating}</b><b className="tool-artifact-chance">TRUE {(tool.trueArtifactChance*100).toFixed(2)}%</b>{tool.ghostCapable&&<b>GHOST READY</b>}</div>{recipe&&open?<div className="tool-cost">{recipe.processedInputs.map(i=><b key={i.id}>{craftingName(i.id)} ×{i.quantity}</b>)}{recipe.mineralInputs.map(i=><b className="mineral-cost" key={i.id}>{craftingName(i.id)} ×{i.quantity}</b>)}{(recipe.oreInputs||[]).map(i=><b key={i.id}>{ghostOreConcealed(save,i.id)?"UNKNOWN ORE":craftingName(i.id)} ×{i.quantity}</b>)}{recipe.dustCost?<b>Specimen Dust ×{recipe.dustCost}</b>:null}</div>:<div className="tool-cost"><b>{tool.tier===0?"STARTING TOOL":"RECIPE REVEALS AFTER PREVIOUS TIER"}</b></div>}<button className={!owned&&!ready?"unavailable":""} aria-disabled={!owned&&!ready} onClick={act}>{save.equippedTool===tool.id?"EQUIPPED":owned?"EQUIP":open?(ready?"CRAFT":"VIEW UPGRADE PATH"):"🔒 BLUEPRINT LOCKED"}</button></article>
    })}</div></section>
