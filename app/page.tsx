...
import { CANONICAL_EXCAVATION_PROBABILITIES, canAfford, equippedTrueArtifactChance, forgeAtomic, forgeRecipes, forgedItems, metallurgyRecipes, processedMaterials, smeltAtomic, type RecipeUnlock, type ResourceState } from "./metallurgy";
import { DEFAULT_TOOL_SKIN_ID, toolSkin, toolSkins } from "./tool-skins";
...
type Save = { digs: number; emptyDigs:number; strikes: number; distance: number; combos: Record<string, number>; ores: Record<string, number>; oreResources:Record<string,number>;mineralResources:Record<string,number>;processedResources:Record<string,number>;ownedTools:string[];equippedTool:string;toolTier:number;toolSkinId:string; minerals: Record<string, number>; first: Record<string, number>; achievements: string[]; streak: number; longestStreak:number;newStreak:number;longestNewStreak:number; dust: number; dustEarned:number;dustSpent:number; biome: Biome; unlockedBiomes:Biome[]; completedBiomes:Biome[]; milestones: Record<string, number>; lastDigAt: number; schema: number; settings:Settings;unlocks:string[];equipped:string;huntTarget:string|null;huntCounts:Record<string,number>;huntStartedAtDig:number;longestHunt:number;trueArtifacts:Record<string,number>;trueFirst:Record<string,number>;activeTrueEncounter:ActiveTrueEncounter|null;forbiddenTunnel:ForbiddenTunnelState|null;pendingArtifactModifier:ArtifactModifier|null;misses:number;veinOre:string|null;veinDigsRemaining:number;milestoneDigs:Record<string,number> };
...
const migrate=(old:any):Save=>{
  ...
  const toolTier=Math.max(0,...ownedTools.map((id:string)=>forgedItems.find(t=>t.id===id)?.tier||0)),toolSkinId=toolSkin(old.toolSkinId).id;
  const encounter=old.activeTrueEncounter,validEncounter=encounter&&trueArtifactPool.some(a=>a.id===encounter.artifactId)&&["empty","ore"].includes(encounter.trigger)&&Number(encounter.maxHp)>0?{artifactId:String(encounter.artifactId),trigger:encounter.trigger as "empty"|"ore",hp:Math.max(1,Math.min(Number(encounter.hp)||1,Number(encounter.maxHp))),maxHp:Number(encounter.maxHp),startedAtDig:Number(encounter.startedAtDig)||Number(old.digs||0)+1}:null;
  ...
  const provisional={...blank,...old,combos,ores:oreCounts,oreResources,mineralResources,processedResources,ownedTools,equippedTool,toolTier,toolSkinId,first,minerals:mineralCounts,activeTrueEncounter:validEncounter,forbiddenTunnel,pendingArtifactModifier,settings:{...defaultSettings,...old.settings},schema:15} as Save;
...
export default function Home() {
  ...
  const strike = (point?:{x:number;y:number}) => {
    ...
    if (stage === "tunnel") {
      ...
      const modifier=save.pendingArtifactModifier,normalChance=equippedTrueArtifactChance(equippedMiningTool(save)),trueChance=artifactChanceForDig(normalChance,modifier),artifactWon=rng.current()<trueChance;
      if(modifier&&!modifier.consumed)setSave(s=>({...s,pendingArtifactModifier:markModifierRolled(s.pendingArtifactModifier)}));
      if(artifactWon){beginTrueEncounter(pickTrue(rng.current),"empty");return;}
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      if(tab === "mine" && !found && !trueFind){
        ...
        if(tool?.holdToMine&&tool.continuousMining){setMiningEngaged(true);autoMineTimer.current=setInterval(()=>strikeRef.current(),tool.intervalMs||tool.actionDurationMs);}
      }
    };
  ...
  const activeSkin=toolSkin(save.toolSkinId);
  ...
  return <main className={`tab-${tab} ${save.settings.reducedMotion?"reduced-motion":""} ${save.settings.reducedShake?"reduced-shake":""} ${save.settings.highContrast?"high-contrast":""} cosmetic-${save.equipped}`}>
    ...
    {tab === "mine" && <section className={`mine-screen biome-${save.biome} ${impact ? "screen-hit" : ""} stage-${stage}`} style={{"--biome-accent":biomeVisuals[save.biome].accent,"--biome-secondary":biomeVisuals[save.biome].secondary,"--biome-canvas":biomeVisuals[save.biome].canvas,"--biome-cavity":biomeVisuals[save.biome].cavity,"--biome-debris":biomeVisuals[save.biome].debris,"--biome-particle":biomeVisuals[save.biome].particle,"--biome-light":biomeVisuals[save.biome].light} as React.CSSProperties}>
      ...
      <div className={`dig-panel ${stage==="artifact"?"artifact-dig-panel":""}`}><div><span className="mouse-icon">↙</span><strong>{stage === "artifact" ? "KEEP HITTING IT" : stage === "ore" ? "CRACK DEPOSIT" : "CLICK TO STRIKE"}</strong><small>or press SPACE</small></div><div className="integrity">{stage === "artifact" ? <div className="artifact-integrity-head"><span><small>ANOMALOUS INTEGRITY</small><strong>{Math.ceil(rockHp)} <em>/ {maxHp}</em></strong></span><b className="artifact-multiplier">20×<small>RESISTANCE</small></b></div> : <span>{stage === "ore" ? "ORE SHELL" : <>TUNNEL PROGRESS · {Math.round((1-rockHp/maxHp)*100)}% · <b className={`depth-band depth-${depthBand(maxHp)}`}>{depthBand(maxHp).toUpperCase()}</b></>}</span>}<i>{Array.from({length: 12},(_,i)=><b key={i} className={i < Math.ceil((rockHp/maxHp)*12) ? "full" : ""}/>)}</i></div></div>
...
function ForbiddenTunnelEncounter({state,reducedMotion,onFirst,onSecond,onContinue}:{state:ForbiddenTunnelState;reducedMotion:boolean;onFirst:(direction:FirstDirection)=>void;onSecond:(direction:SecondDirection)=>void;onContinue:()=>void}){
  ...
  return <div className={`forbidden-tunnel-overlay chamber-${state.chamber} ${state.resolution==="deep"?"deep-victory":""} ${state.resolution==="sealed"?"sealed-result":""} ${reducedMotion?"reduced-motion":""}`} role="dialog" aria-modal="true" aria-label="Forbidden Tunnel encounter"><section className="forbidden-tunnel-card">
    ...
    {state.chamber==="first"&&<div className="tunnel-passages first-passages">{(["left","middle","right"] as FirstDirection[]).map(d=><button key={d} onClick={()=>onFirst(d)}><i aria-hidden="true"/><strong>{d.toUpperCase()}</strong><small>ENTER PASSAGE</small></button>)}</div>}
...
function Forge({save,setSave}:{save:Save;setSave:React.Dispatch<React.SetStateAction<Save>>}){
  ...
  const [confirmation,setConfirmation]=useState<{kind:"smelt"|"forge";id:string}|null>(null);
  ...
  return <section className="forge-page"><div className="workshop-smoke" aria-hidden="true"/><div className="workshop-header"><aside className="goblin-corner" aria-hidden="true"><span className="goblin-head">⌁</span><p>PEON SMART.<br/>PEON BUILD.<br/>PEON DIG DEEP.<br/><b>ZUG ZUG!</b></p></aside><div className="workshop-sign"><span>PEON TECHNOLOGY</span><small>IF IT BONKS, IT WORKS.</small></div><div className="equipped-board equipped-loadout"><div className="loadout-technology"><span>TECHNOLOGY · GAMEPLAY</span><strong>{current.name}</strong><small>{current.technicalName}</small><ToolArt tool={current}/></div><div className="loadout-model"><span>MODEL · APPEARANCE</span><strong>{selectedSkin.name}</strong><small>{selectedSkin.technicalName}</small><img src={selectedSkin.artwork} alt=""/></div><footer><b>TRUE <em>{(equippedTrueArtifactChance(current)*100).toFixed(2)}%</em></b><b>ACTION <em>{current.actionDurationMs}ms</em></b><b>IMPACT <em>{current.damage.toFixed(2)}×</em></b></footer></div></div>
    ...
    <section className="skin-locker"><h3>PICKAXE RACK <small>MODEL ONLY · TECHNOLOGY STATS STAY EQUIPPED</small></h3><div className="skin-rack">{toolSkins.map(skin=><article key={skin.id} className={`${save.toolSkinId===skin.id?"selected":""} skin-card-${skin.silhouette}`}><div className="skin-preview"><img src={skin.artwork} alt=""/></div><span>COSMETIC MODEL</span><strong>{skin.name}</strong><small className="skin-technical">{skin.technicalName}</small><p>{skin.flavor}</p>{skin.barkSequence?<blockquote>{skin.barkSequence.map((beat,index)=><span key={beat.text}>PEON: “{beat.text}”{index<skin.barkSequence!.length-1&&<i>…</i>}</span>)}</blockquote>:skin.bark&&<blockquote>PEON: “{skin.bark}”</blockquote>}<button onClick={()=>{setSave(s=>({...s,toolSkinId:skin.id}));notify("success",`${skin.name} SELECTED`,`Appearance changed. ${current.name} technology and all gameplay stats remain equipped.`)}}>{save.toolSkinId===skin.id?"ON THE RACK":"USE THIS MODEL"}</button></article>)}</div></section>
    <section className="arsenal"><h3>FORGED MINING TOOLS <small>BIGGER = BETTER</small></h3><div className="tool-progression">{forgedItems.map(tool=>{
      ...
      const act=()=>{if(owned){setSave(s=>({...s,equippedTool:tool.id}));return notify("success",tool.name,save.equippedTool===tool.id?"Already equipped. Peon confirms bonking readiness.":`${tool.technicalName} equipped. Cosmetic unchanged. TRUE chance ${(equippedTrueArtifactChance(tool)*100).toFixed(2)}%.`)}if(!open)return notify("error","BLUEPRINT LOCKED","Forge the previous tool and reach the required mine first.");if(!ready&&recipe){track("recipe_blocked",{recipe_id:recipe.id,processed_missing:missing(save.processedResources,recipe.processedInputs),mineral_missing:missing(save.mineralResources,recipe.mineralInputs)});return notify("error","MATERIALS REQUIRED",`Processed: ${missing(save.processedResources,recipe.processedInputs)||"ready"}. Minerals: ${missing(save.mineralResources,recipe.mineralInputs)||"ready"}.`)}if(recipe){track("recipe_viewed",{recipe_id:recipe.id,ready:true});setConfirmation({kind:"forge",id:recipe.id})}};
      return <article className={`${owned?"owned":""} ${save.equippedTool===tool.id?"equipped":""} ${!open?"planned":""}`} key={tool.id}><span>TIER {tool.tier} · {tool.inputMode.toUpperCase()}</span><h3>{tool.name}</h3><h4>{tool.technicalName}</h4><ToolArt tool={tool}/><div className="tool-stats"><b>{(equippedTrueArtifactChance(tool)*100).toFixed(2)}%</b><b>{tool.actionDurationMs}ms</b><b>{tool.damage.toFixed(2)}×</b></div>{recipe&&open?<div className="tool-cost">{recipe.processedInputs.map(i=><b key={i.id}>{craftingName(i.id)} ×{i.quantity}</b>)}{recipe.mineralInputs.map(i=><b className="mineral-cost" key={i.id}>{craftingName(i.id)} ×{i.quantity}</b>)}</div>:<div className="tool-cost"><b>{tool.tier===0?"STARTING TOOL":"RECIPE REVEALS AFTER PREVIOUS TIER"}</b></div>}<button className={!owned&&!ready?"unavailable":""} aria-disabled={!owned&&!ready} onClick={act}>{save.equippedTool===tool.id?"EQUIPPED":owned?"EQUIP":open?(ready?"CRAFT":"MATERIALS REQUIRED"):"🔒 BLUEPRINT LOCKED"}</button></article>
    })}</div></section>
    ...
    {confirmation&&<CraftConfirmation save={save} action={confirmation} onCancel={()=>setConfirmation(null)} onConfirm={()=>confirmation.kind==="smelt"?runMetallurgy(confirmation.id):runForge(confirmation.id)}/>}
  </section>
}

function CraftConfirmation({save,action,onCancel,onConfirm}:{save:Save;action:{kind:"smelt"|"forge";id:string};onCancel:()=>void;onConfirm:()=>void}){
  ...
  const rows=smelt?smelt.inputs.map(i=>({i,stock:save.oreResources[i.id]})):[...(forge?.processedInputs||[]).map(i=>({i,stock:save.processedResources[i.id]})),...(forge?.mineralInputs||[]).map(i=>({i,stock:save.mineralResources[i.id]}))];
  return <div className="craft-confirm-overlay" role="dialog" aria-modal="true" aria-label="Confirm resource transaction"><section className="craft-confirm"><p>GOBLIN TRANSACTION CONTROL</p><h2>{smelt?smelt.name:`FORGE ${tool?.name}`}</h2><div className="craft-consumption"><header><span>MATERIAL</span><span>CURRENT</span><span>AFTER</span></header>{rows.map(({i,stock})=><div key={i.id}><strong>{craftingName(i.id)}</strong><span>{stock||0}</span><b>{(stock||0)-i.quantity}</b></div>)}</div><div className="craft-product"><small>PRODUCT CREATED</small><strong>{smelt?`${craftingName(smelt.outputId)} ×${smelt.outputQuantity}`:`TIER ${tool?.tier} · ${tool?.name}`}</strong></div><footer><button onClick={onCancel}>CANCEL</button><button className="confirm" onClick={onConfirm}>CONFIRM &amp; CONSUME</button></footer></section></div>;
}
...
function Reveal({ found, total, attempt, biome, onContinue }: { found: {ore:Item;mineral:Item;isNew:boolean;count:number}; total:number;attempt:number;biome:Biome; onContinue:()=>void }) {
  ...
  return <div className={`reveal ore-${found.ore.id} ${huge?"mythic":""} ${rareDiscovery?"rare-discovery":""} ${!found.isNew&&!huge?"quick":""}`}><div className="reveal-card"><button className="close" onClick={onContinue}>×</button>{rareDiscovery&&<div className="discovery-ribbon"><span>ORE WHORE · VOLUME I</span><strong>RARE DISCOVERY</strong></div>}<p className="eyebrow">{personality}</p><div className="combo-art"><img className="big-gem ore ore-sprite ore-sprite-reveal" src={oreAsset(found.ore.id)} alt=""/><b>+</b><img className="big-gem mineral mineral-sprite mineral-sprite-reveal" src={mineralAsset(found.mineral.id)} alt=""/></div><div className="names"><div><small>{found.ore.rarity}</small><strong>{found.ore.name}</strong></div><b>CONTAINING</b><div><small>{found.mineral.rarity}</small><strong>{found.mineral.name}</strong></div></div><div className={`verdict ${found.isNew?"new":"duplicate"}`}><span>{found.isNew ? (huge?"MYTHIC DISCOVERY":rareDiscovery?"RARE DISCOVERY":"NEW COMBINATION") : `DUPLICATE ×${found.count}`}</span><strong>{found.isNew ? `${total} / 225` : `+${dustByRarity[found.mineral.rarity]} SPECIMEN DUST`}</strong><small>NATURAL ODDS · APPROX. 1 IN {comboOdds.toLocaleString()}</small></div>{rareDiscovery&&<div className="discovery-meta"><span>ATTEMPT #{attempt}</span><span>ALBUM {total}/225</span></div>}<button className="continue" onClick={onContinue}>{found.isNew?"CONTINUE MINING":"AGAIN. NOW."} <span>→</span></button></div></div>;
}
