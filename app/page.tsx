...
type Save = { digs: number; emptyDigs:number; strikes: number; distance: number; combos: Record<string, number>; ores: Record<string, number>; rawResources:Record<string,number>;processedMaterials:Record<string,number>;ownedTools:string[];equippedTool:string; minerals: Record<string, number>; first: Record<string, number>; achievements: string[]; streak: number; longestStreak:number;newStreak:number;longestNewStreak:number; dust: number; dustEarned:number;dustSpent:number; biome: Biome; unlockedBiomes:Biome[]; completedBiomes:Biome[]; milestones: Record<string, number>; lastDigAt: number; schema: number; settings:Settings;unlocks:string[];equipped:string;huntTarget:string|null;huntCounts:Record<string,number>;huntStartedAtDig:number;longestHunt:number;trueArtifacts:Record<string,number>;trueFirst:Record<string,number>;activeTrueEncounter:ActiveTrueEncounter|null;misses:number;veinOre:string|null;veinDigsRemaining:number;milestoneDigs:Record<string,number> };
...
function Wanted({save,onHunt}:{save:Save;onHunt:(b:Biome,ore:string)=>void}){
  ...
  return <section className="page wanted-page">
    ...
    <div className="wanted-list">{shown.map(r=><article className={r.found===14?`last-one last-one-${lastOneTier(save,r.ore.id)}`:""} key={r.ore.id}><header><img className="ore-gem ore-sprite ore-sprite-wanted" src={oreAsset(r.ore.id)} alt=""/><div><small>{r.found===14?lastOneCopy(save,r.ore.id):"INCOMPLETE PAGE"}</small><h3>{r.ore.name} — {r.found}/15</h3></div></header><div className="missing-grid volume-missing">{r.missing.map(m=>{const target=`${r.ore.id}-${m.id}`,b=best(r.ore);return <div className={save.huntTarget===target?"pinned":""} key={m.id}><img className="wanted-gem mineral-sprite mineral-sprite-wanted" src={mineralAsset(m.id)} alt=""/><strong>{m.name}</strong><small>{biomeNames[b]} · 1/{odds(r.ore,m,b)}</small><button onClick={()=>onHunt(b,target)}>{save.huntTarget===target?"PINNED":"PIN TARGET"}</button></div>})}</div><footer><span>RECOMMENDED: {biomeNames[best(r.ore)]}</span><button onClick={()=>{const m=[...r.missing].sort((a,b)=>a.weight-b.weight)[0];onHunt(best(r.ore),`${r.ore.id}-${m.id}`)}}>HUNT THIS PAGE →</button></footer></article>)}</div>
  </section>
}
