...
type Save = { digs: number; strikes: number; distance: number; combos: Record<string, number>; ores: Record<string, number>; minerals: Record<string, number>; first: Record<string, number>; achievements: string[]; streak: number; longestStreak:number;newStreak:number;longestNewStreak:number; dust: number; dustEarned:number;dustSpent:number; biome: Biome; unlockedBiomes:Biome[]; milestones: Record<string, number>; lastDigAt: number; schema: number; settings:Settings;unlocks:string[];equipped:string;huntTarget:string|null;huntCounts:Record<string,number>;huntStartedAtDig:number;longestHunt:number };
...
const blank: Save = { digs: 0, strikes: 0, distance: 0, combos: {}, ores: {}, minerals: {}, first: {}, achievements: [], streak: 0,longestStreak:0,newStreak:0,longestNewStreak:0, dust: 0,dustEarned:0,dustSpent:0, biome: "old", unlockedBiomes:["old"], milestones: {}, lastDigAt: 0, schema: 5,settings:defaultSettings,unlocks:[],equipped:"standard",huntTarget:null,huntCounts:{},huntStartedAtDig:0,longestHunt:0 };
...
export default function Home() {
  ...
  useEffect(()=>{if(!loaded)return;const next=[...save.unlockedBiomes];for(let i=0;i<biomeOrder.length-1;i++){if(next.includes(biomeOrder[i])&&biomeComplete(save,biomeOrder[i])&&!next.includes(biomeOrder[i+1]))next.push(biomeOrder[i+1]);}if(next.length!==save.unlockedBiomes.length){const newest=biomeOrder.filter(b=>next.includes(b)&&!save.unlockedBiomes.includes(b)).at(-1)!;setSave(s=>({...s,unlockedBiomes:biomeOrder.filter(b=>next.includes(b))}));setToast({name:`${biomeNames[newest]} UNLOCKED`,text:`The previous mine is complete. ${biomeNames[newest]} is now open.`});track("biome_unlocked",{biome:newest});}},[loaded,save.combos,save.unlockedBiomes]);
