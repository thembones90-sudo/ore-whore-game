...
type Settings = { master:number;sfx:number;musicEnabled:boolean;pickaxeSfxEnabled:boolean;reducedShake:boolean;reducedMotion:boolean;vibration:boolean;highContrast:boolean;helpSeen:boolean };
...
const defaultSettings:Settings={master:.7,sfx:.8,musicEnabled:true,pickaxeSfxEnabled:true,reducedShake:false,reducedMotion:false,vibration:true,highContrast:false,helpSeen:false};
...
const migrate=(old:any):Save=>{
  ...
  const trueArtifacts:Record<string,number>={};for(const artifact of trueArtifactPool)if(Number(old.trueArtifacts?.[artifact.id]||0)>0)trueArtifacts[artifact.id]=1;
...
export default function Home() {
  ...
  useEffect(()=>{const audio=soundtrackRef.current;if(!audio)return;audio.volume=Math.min(1,Math.max(0,save.settings.master*.42));if(!loaded||!save.settings.musicEnabled){audio.pause();return}audio.play().catch(()=>{/* Browser will resume it on the next user gesture. */})},[loaded,save.settings.master,save.settings.musicEnabled]);
  useEffect(()=>{const resume=()=>{const audio=soundtrackRef.current;if(save.settings.musicEnabled&&audio?.paused)audio.play().catch(()=>{})};window.addEventListener("pointerdown",resume,{passive:true});window.addEventListener("keydown",resume);return()=>{window.removeEventListener("pointerdown",resume);window.removeEventListener("keydown",resume)}},[save.settings.musicEnabled]);
  ...
  const strike = (point?:{x:number;y:number}) => {
    ...
    if (stage === "tunnel") {
      ...
      const modifier=save.pendingArtifactModifier,trueChance=artifactChanceForDig(modifier,equippedMiningTool(save).trueArtifactChance),artifactWon=rng.current()<trueChance;
...
function TrueArchive({save}:{save:Save}){
  ...
  const equipped=equippedMiningTool(save),effectiveChance=artifactChanceForDig(save.pendingArtifactModifier,equipped.trueArtifactChance);
  return <><section className="page true-archive-page">
    ...
    <p className="true-archive-intro">Common through Legendary belongs to the mountain. These do not. Equipped tool: <strong>{equipped.name}</strong>. Current TRUE Artifact chance: <strong>{(effectiveChance*100).toFixed(2)}%</strong> · approximately 1 in {Math.round(1/effectiveChance).toLocaleString()}. Better Peon Technology raises the ordinary per-excavation chance up to 1.00%. Forbidden Tunnel modifiers apply to the next completed dig only. No pity system. No stacking.{totalFound?` Total anomalies logged: ${totalFound}.`:""}</p>
