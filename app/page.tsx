...
const MINE_UNLOCK_RATIO=.6;
const biomeDiscoveries=(save:Save,biome:Biome)=>biomePages[biome].reduce((total,id)=>total+minerals.filter(m=>save.combos[`${id}-${m.id}`]).length,0);
const biomeDiscoveryTotal=(biome:Biome)=>biomePages[biome].length*minerals.length;
const biomeUnlockTarget=(biome:Biome)=>Math.ceil(biomeDiscoveryTotal(biome)*MINE_UNLOCK_RATIO);
const biomeProgressionReached=(save:Save,biome:Biome)=>biomeDiscoveries(save,biome)>=biomeUnlockTarget(biome);
...
export default function Home() {
  ...
  // eslint-disable-next-line react-hooks/set-state-in-effect, react-hooks/exhaustive-deps -- derives mine-completion/unlock from save state; deps intentionally narrowed to save.combos/unlockedBiomes/completedBiomes so this only re-evaluates on progression changes, not every dig. Moving this into the dig-mutation call site would touch protected progression logic (see HANDOFF_FOR_CLAUDE.md).
  useEffect(()=>{if(!loaded||mineCompletion)return;const completed=biomeOrder.find((b,i)=>save.unlockedBiomes.includes(b)&&(i===biomeOrder.length-1?biomeComplete(save,b):biomeProgressionReached(save,b))&&!save.completedBiomes.includes(b));if(!completed)return;const i=biomeOrder.indexOf(completed),next=i<biomeOrder.length-1?biomeOrder[i+1]:undefined;setSave(s=>({...s,completedBiomes:[...s.completedBiomes,completed],unlockedBiomes:next&&!s.unlockedBiomes.includes(next)?[...s.unlockedBiomes,next]:s.unlockedBiomes}));setMineCompletion({completed,next});track("biome_completed",{biome:completed,digs:save.digs,pages:completedPages(save,completed),discoveries:biomeDiscoveries(save,completed)});emitGameplayEvent("MINE_COMPLETED",{biome:completed});if(next)track("biome_unlocked",{biome:next});},[loaded,save.combos,save.unlockedBiomes,save.completedBiomes,mineCompletion]);
  ...
  return <main className={`${save.settings.reducedMotion?"reduced-motion":""} ${save.settings.reducedShake?"reduced-shake":""} ${save.settings.highContrast?"high-contrast":""} cosmetic-${save.equipped}`}>
    ...
    {tab === "mine" && <section className={`mine-screen biome-${save.biome} ${impact ? "screen-hit" : ""} stage-${stage}`} style={{"--biome-accent":biomeVisuals[save.biome].accent,"--biome-secondary":biomeVisuals[save.biome].secondary,"--biome-canvas":biomeVisuals[save.biome].canvas,"--biome-cavity":biomeVisuals[save.biome].cavity,"--biome-debris":biomeVisuals[save.biome].debris,"--biome-particle":biomeVisuals[save.biome].particle,"--biome-light":biomeVisuals[save.biome].light} as React.CSSProperties}>
      ...
      {(()=>{const final=save.biome==="northrend",found=biomeDiscoveries(save,save.biome),target=biomeUnlockTarget(save.biome),pages=completedPages(save,save.biome),pageTotal=biomePages[save.biome].length,reached=final?biomeComplete(save,save.biome):biomeProgressionReached(save,save.biome),progress=final?pages/pageTotal*100:Math.min(100,found/target*100);return <div className="mine-mastery"><div><span>{biomeNames[save.biome]} {final?"MASTERY":"DISCOVERY"}</span><strong>{final?`${pages} / ${pageTotal} PAGES`:`${found} / ${target} TO DESCEND`}</strong></div><i><b style={{width:`${progress}%`}}/></i><small>{reached?(final?"MINE COMPLETE":`PASSAGE OPEN · ${pages}/${pageTotal} pages mastered`):(final?`${pageTotal-pages} pages remain to master Volume I.`:`${target-found} discoveries remain · full mastery is optional.`)}</small></div>})()}
      <div className="biomes volume-biomes" aria-label="Mine location">{biomeOrder.map((b,i)=>{const open=save.unlockedBiomes.includes(b),done=completedPages(save,b),previous=i?biomeOrder[i-1]:b,v=biomeVisuals[b];return <button key={b} disabled={!open} style={{"--card-accent":v.accent,"--card-secondary":v.secondary,"--card-bg":v.card} as React.CSSProperties} className={`mine-card biome-card-${b} ${save.biome===b?"chosen":""} ${open?"":"locked"}`} onClick={()=>{setSave(s=>({...s,biome:b}));track("biome_selected",{biome:b})}}><span>{open?biomeNames[b]:`🔒 ${biomeNames[b]}`}</span><small>{open?`${done}/${biomePages[b].length} PAGES · ${distributionLabel(b)}`:`DISCOVER ${biomeUnlockTarget(previous)} IN ${biomeNames[previous]} · ${biomeDiscoveries(save,previous)}/${biomeUnlockTarget(previous)}`}</small></button>})}</div>
...
function MineCompletion({data,digs,onContinue}:{data:{completed:Biome;next?:Biome};digs:number;onContinue:()=>void}){
 return <div className="mine-completion"><div><p className="eyebrow">{data.next?"DESCENT THRESHOLD REACHED":"ALL PAGES COMPLETE"}</p><div className="completion-seal">◆</div><h2>{biomeNames[data.completed]}<br/><i>{data.next?"CLEARED.":"MASTERED."}</i></h2><p>{data.next?`${biomeUnlockTarget(data.completed)} unique combinations documented after ${digs} total deposits. Full mastery remains optional.`:`${biomePages[data.completed].length} pages sealed after ${digs} total deposits. The mountain has reluctantly acknowledged your paperwork.`}</p>{data.next?<div className="next-mine-reveal"><small>NEW DESCENT UNLOCKED</small><strong>{biomeNames[data.next]}</strong></div>:<div className="next-mine-reveal"><small>VOLUME I MINE NETWORK</small><strong>FULLY MASTERED</strong></div>}<button onClick={onContinue}>{data.next?`ENTER ${biomeNames[data.next]}`:"RETURN TO THE MOUNTAIN"} <span>→</span></button></div></div>
}
