...
export default function Home() {
  ...
  useEffect(()=>{if(!loaded)return;const n=Object.keys(save.combos).length;const rewards:[[number,string],[number,string],[number,string],[number,string],[number,string],[number,string]]=[[23,"rust"],[57,"gilded"],[113,"menace"],[169,"khoriumframe"],[203,"titan"],[225,"volumeone"]];const earned=rewards.filter(x=>n>=x[0]).map(x=>x[1]).filter(id=>!save.unlocks.includes(id));if(earned.length)setSave(s=>({...s,unlocks:[...s.unlocks,...earned]}))},[loaded,save.combos,save.unlocks]);
  ...
  return <main className={`${save.settings.reducedMotion?"reduced-motion":""} ${save.settings.reducedShake?"reduced-shake":""} ${save.settings.highContrast?"high-contrast":""} cosmetic-${save.equipped}`}>
    ...
    {tab === "mine" && <section className={`mine-screen ${impact ? "screen-hit" : ""} stage-${stage}`}>
      ...
      <div className="biomes volume-biomes" aria-label="Mine location">{(Object.keys(biomeNames) as Biome[]).map(b=><button key={b} className={save.biome===b?"chosen":""} onClick={()=>{setSave(s=>({...s,biome:b}));track("biome_selected",{biome:b})}}><span>{biomeNames[b]}</span><small>{distributionLabel(b)}</small></button>)}</div>
    ...
    {milestone && <Milestone data={milestone} biome={save.biome} onClose={()=>setMilestone(null)} onAlbum={()=>{if(milestone.level===14&&milestone.missing){const target=`${milestone.ore.id}-${milestone.missing.id}`,b=bestBiome(milestone.ore);setSave(s=>({...s,biome:b,huntTarget:target}));track("hunt_started",{biome:b,combination_id:target});setTab("mine")}else setTab("album");setMilestone(null)}} />}
...
function Wanted({save,onHunt}:{save:Save;onHunt:(b:Biome,ore:string)=>void}){
  ...
  const shown=rows.filter(r=>mine==="all"||bestBiome(r.ore)===mine);
  return <section className="page wanted-page"><div className="page-head"><div><p className="eyebrow">SPECIFIC REASONS TO KEEP SUFFERING</p><h2>MISSING <i>SPECIMENS</i></h2></div><div className="completion"><span>STILL HIDING</span><strong>{225-Object.keys(save.combos).length}</strong></div></div><div className="wanted-sort"><span>SORT</span>{[["closest","CLOSEST"],["rarest","RAREST"],["easiest","EASIEST"],["ore","ORE"]].map(x=><button className={sort===x[0]?"active":""} key={x[0]} onClick={()=>setSort(x[0])}>{x[1]}</button>)}<select value={mine} onChange={e=>setMine(e.target.value as Biome|"all")}><option value="all">ALL MINES</option>{(Object.keys(biomeNames) as Biome[]).map(b=><option value={b} key={b}>{biomeNames[b]}</option>)}</select></div><div className="wanted-list">{shown.map(r=><article className={r.found===14?"last-one":""} key={r.ore.id}><header><span className="ore-gem" style={{"--gem":r.ore.color} as React.CSSProperties}>◆</span><div><small>{r.found===14?"ONE SPECIMEN REMAINS":"INCOMPLETE PAGE"}</small><h3>{r.ore.name} — {r.found}/15</h3></div></header><div className="missing-grid volume-missing">{r.missing.map(m=>{const target=`${r.ore.id}-${m.id}`,best=bestBiome(r.ore);return <div className={save.huntTarget===target?"pinned":""} key={m.id}><span style={{color:m.color}}>◆</span><strong>{m.name}</strong><small>{biomeNames[best]} · 1/{odds(r.ore,m,best)}</small><button onClick={()=>onHunt(best,target)}>{save.huntTarget===target?"PINNED":"PIN TARGET"}</button></div>})}</div><footer><span>RECOMMENDED: {biomeNames[bestBiome(r.ore)]}</span><button onClick={()=>{const m=[...r.missing].sort((a,b)=>a.weight-b.weight)[0];onHunt(bestBiome(r.ore),`${r.ore.id}-${m.id}`)}}>HUNT THIS PAGE →</button></footer></article>)}</div></section>
}
