...
import { CANONICAL_EXCAVATION_PROBABILITIES, canAfford, forgeAtomic, forgeRecipes, forgedItems, maxCraftable, metallurgyRecipes, processedMaterials, smeltBatchAtomic, type RecipeUnlock, type ResourceState } from "./metallurgy";
...
function CraftConfirmation({save,action,onCancel,onConfirm}:{save:Save;action:{kind:"smelt"|"forge";id:string};onCancel:()=>void;onConfirm:()=>void}){
  ...
  const rows=smelt?smelt.inputs.map(i=>({i:{...i,quantity:i.quantity*batch},stock:save.oreResources[i.id]})):[...(forge?.processedInputs||[]).map(i=>({i,stock:save.processedResources[i.id]})),...(forge?.mineralInputs||[]).map(i=>({i,stock:save.mineralResources[i.id]}))];
  return <div className="craft-confirm-overlay" role="dialog" aria-modal="true" aria-label="Confirm resource transaction"><section className="craft-confirm"><p>GOBLIN TRANSACTION CONTROL</p><h2>{smelt?smelt.name:`FORGE ${tool?.name}`}</h2>{batch>1&&<div className="craft-batch-notice"><small>INDUSTRIAL BATCH</small><strong>MAXIMUM AFFORDABLE RUN ×{batch}</strong></div>}<div className="craft-consumption"><header><span>MATERIAL</span><span>CURRENT</span><span>AFTER</span></header>{rows.map(({i,stock})=><div key={i.id}><strong>{craftingName(i.id)}</strong><span>{stock||0}</span><b>{(stock||0)-i.quantity}</b></div>)}</div><div className="craft-product"><small>PRODUCT CREATED</small><strong>{smelt?`${craftingName(smelt.outputId)} ×${smelt.outputQuantity*batch}`:`TIER ${tool?.tier} · ${tool?.name}`}</strong></div><footer><button onClick={onCancel}>CANCEL</button><button className="confirm" onClick={onConfirm}>{batch>1?`BUILD ALL ×${batch}`:"CONFIRM & CONSUME"}</button></footer></section></div>;
}
