...
export default function Home() {
  ...
  const followPointer=(event:React.PointerEvent<HTMLButtonElement>)=>{if(event.pointerType!=="mouse")return;const rect=event.currentTarget.getBoundingClientRect(),x=Math.max(4,Math.min(96,(event.clientX-rect.left)/rect.width*100)),y=Math.max(8,Math.min(94,(event.clientY-rect.top)/rect.height*100));event.currentTarget.style.setProperty("--pick-x",`${x}%`);event.currentTarget.style.setProperty("--pick-y",`${y}%`);event.currentTarget.classList.add("pick-following")};
...
function ShareCard({data,biome,onClose}:{data:{ore:Item;mineral:Item;attempt:number;total:number};biome:Biome;onClose:()=>void}){return <div className="share-wrap"><div className="share-card"><p>ORE WHORE · VOLUME I</p><div><img className="ore-sprite ore-sprite-share" src={oreAsset(data.ore.id)} alt=""/><b>+</b><img className="mineral-sprite mineral-sprite-share" src={mineralAsset(data.mineral.id)} alt=""/></div><small>{data.ore.rarity.toUpperCase()} + {data.mineral.rarity.toUpperCase()}</small><h2>{data.ore.name}<br/>+ {data.mineral.name}</h2><strong>RARE DISCOVERY</strong><footer><span>NATURAL ODDS · 1 / {odds(data.ore,data.mineral,biome)}</span><span>ATTEMPT #{data.attempt} · ALBUM {data.total}/225</span></footer></div><p>Screenshot this. Nobody will believe you, but try.</p><button onClick={onClose}>CONTINUE</button></div>}
...
function Reveal({ found, total, biome, onContinue }: { found: {ore:Item;mineral:Item;isNew:boolean;count:number}; total:number;biome:Biome; onContinue:()=>void }) {
  ...
  return <div className={`reveal ore-${found.ore.id} ${huge?"mythic":""} ${!found.isNew&&!huge?"quick":""}`}><div className="reveal-card"><button className="close" onClick={onContinue}>×</button><p className="eyebrow">{personality}</p><div className="combo-art"><img className="big-gem ore ore-sprite ore-sprite-reveal" src={oreAsset(found.ore.id)} alt=""/><b>+</b><img className="big-gem mineral mineral-sprite mineral-sprite-reveal" src={mineralAsset(found.mineral.id)} alt=""/></div><div className="names"><div><small>{found.ore.rarity}</small><strong>{found.ore.name}</strong></div><b>CONTAINING</b><div><small>{found.mineral.rarity}</small><strong>{found.mineral.name}</strong></div></div><div className={`verdict ${found.isNew?"new":"duplicate"}`}><span>{found.isNew ? (huge?"MYTHIC DISCOVERY":"NEW COMBINATION") : `DUPLICATE ×${found.count}`}</span><strong>{found.isNew ? `${total} / 225` : `+${dustByRarity[found.mineral.rarity]} SPECIMEN DUST`}</strong><small>NATURAL ODDS · APPROX. 1 IN {comboOdds.toLocaleString()}</small></div><button className="continue" onClick={onContinue}>{found.isNew?"CONTINUE MINING":"AGAIN. NOW."} <span>→</span></button></div></div>;
}
