...
function Album({ save }: { save: Save }) {
  ...
  return <section className="page album-page"><div className="page-head"><div><p className="eyebrow">CLASSIC → TBC → WOTLK</p><h2>VOLUME I <i>ALBUM</i></h2></div><div className="completion"><span>VOLUME COMPLETION</span><strong>{unique} <small>/ 225 · {(unique/225*100).toFixed(1)}%</small></strong></div></div><div className="volume-overview"><span><small>ORES</small>{oreFound}/15</span><span><small>MINERALS</small>{mineralFound}/15</span><span><small>COMPLETED PAGES</small>{completed}/15</span><span><small>MISSING</small>{225-unique}</span></div><div className="overview-highlights"><span><small>RAREST DISCOVERY</small>{globalRare?`${globalRare.o.name} + ${globalRare.m.name} · 1/${globalRare.odd}`:"Nothing worth mentioning yet."}</span><span><small>MOST WANTED</small>{wanted}</span></div>
    <div className="ore-tabs volume-tabs">{ores.map(o => {const n=minerals.filter(m=>save.combos[`${o.id}-${m.id}`]).length;return <button key={o.id} className={`${selected.id === o.id ? "selected" : ""} ${n===15?"complete":""}`} onClick={() => {setSelected(o);track("ore_page_opened",{ore_id:o.id,completion:n/15})}}><img className="ore-tab-sprite" src={oreAsset(o.id)} alt=""/><span>{o.name.replace(" Ore","")}<small>{n}/15 {n===15?"✓":""}</small></span></button>})}</div>
    <div className={`album-title milestone-${total}`}><div><img className="ore-gem ore-sprite ore-sprite-album" src={oreAsset(selected.id)} alt=""/><div><p>{selected.rarity.toUpperCase()} ORE · {save.ores[selected.id]||0} MINED</p><h3>{selected.name} {total===15&&<b className="complete-stamp">PAGE COMPLETE</b>}</h3><small>{total===14?`ONE SPECIMEN REMAINS: ${minerals.find(m=>!save.combos[`${selected.id}-${m.id}`])?.name}. WHERE IS IT?`:selected.note}</small><div className="page-stats">RAREST: {rarest?.m.name||"—"} · MOST DUPLICATED: {most?`${most.m.name} ×${most.count}`:"—"} · MISSING: {15-total}</div></div></div><strong>{total}<small>/15 · {Math.round(total/15*100)}%</small></strong></div>
