...
function Forge({save,setSave}:{save:Save;setSave:React.Dispatch<React.SetStateAction<Save>>}){
  ...
  return <section className="page forge-page"><div className="page-head"><div><p className="eyebrow">DIG → REFINE / ALLOY → FORGE → ITEM</p><h2>PEON <i>TECHNOLOGY</i></h2></div><div className="completion"><span>EQUIPPED TOOL</span><strong>{current.name}</strong><small>{current.technicalName}</small></div></div>
    ...
    <h3 className="section-label">FORGED MINING TOOLS</h3><div className="tool-progression">{forgedItems.map(tool=>{const owned=save.ownedTools.includes(tool.id),recipe=forgeRecipes.find(r=>r.resultingItemId===tool.id),open=!recipe||recipeUnlocked(save,recipe.unlock),ready=!!recipe&&open&&canAfford(save.processedMaterials,recipe.inputs);return <article className={`${owned?"owned":""} ${save.equippedTool===tool.id?"equipped":""} ${tool.planned?"planned":""}`} key={tool.id}><span>TIER {tool.tier} · {tool.inputMode.toUpperCase()}</span><h3>{tool.name}</h3><h4>{tool.technicalName}</h4><div className="tool-stats"><b>TRUE {(equippedTrueArtifactChance(tool)*100).toFixed(2)}%</b><b>{tool.actionDurationMs}ms ACTION</b><b>{tool.damage.toFixed(2)}× IMPACT</b></div><p>{tool.description}</p>{recipe&&<div className="tool-cost">{recipe.inputs.map(i=><b key={i.id}>{craftingName(i.id)} ×{i.quantity}</b>)}</div>}<button disabled={tool.planned||(!owned&&!ready)} onClick={()=>owned?setSave(s=>({...s,equippedTool:tool.id})):recipe&&runForge(recipe.id)}>{tool.planned?"DESIGN PENDING":save.equippedTool===tool.id?"EQUIPPED":owned?"EQUIP":open?(ready?"FORGE TOOL":"MATERIALS REQUIRED"):"BLUEPRINT LOCKED"}</button></article>})}</div>
  </section>
}
