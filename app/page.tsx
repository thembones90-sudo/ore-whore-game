...
export default function Home() {
  ...
  useEffect(()=>{const audio=soundtrackRef.current;if(!audio)return;audio.volume=Math.min(1,Math.max(0,save.settings.master*save.settings.musicVolume));if(!loaded||!save.settings.musicEnabled){audio.pause();return}audio.play().catch(()=>{/* Browser will resume it on the next user gesture. */})},[loaded,save.settings.master,save.settings.musicVolume,save.settings.musicEnabled]);
  ...
  function playImpact(kind:"rock"|"clank"|"crack"|"miss"|"perfect"|"crit"){
    ...
    try{
      if(kind!=="miss"){
        ...
        hit.volume=Math.min(1,save.settings.master*save.settings.sfx);
