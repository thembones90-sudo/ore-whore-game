...
function EndgameExperience({save,startAtCompletion,onFinished,onView,onNewGame}:{save:Save;startAtCompletion:boolean;onFinished:()=>void;onView:()=>void;onNewGame:()=>void}){
  ...
  return <div className={`endgame-overlay endgame-${phase}`} role="dialog" aria-modal="true" aria-label="ORE WHORE ending">
    ...
    {phase==="termination"&&<section className="endgame-termination"><small>DEPARTMENT OF UNLICENSED EXCAVATION</small><h2>EMPLOYMENT STATUS<br/><strong>TERMINATED</strong></h2><div><span>CONTRACT STATUS</span><b>SUPERSEDED</b></div><p>CONTRACT SUPERSEDED BY INVITATION</p><blockquote className="shadez-voiced"><ShadezAvatar/><span><small>FOREMAN SHADEZ</small>Your contract is no longer mine to enforce.<br/><b>Proceed.</b></span></blockquote><button onClick={()=>setPhase("story")}>THE LAST FIND →</button></section>}
...
function Onboarding({agreementRequired,onDone}:{agreementRequired:boolean;onDone:(signed:boolean)=>void}){
  const [phase,setPhase]=useState<"title"|"management"|"contract"|"signing">("title"),[declined,setDeclined]=useState(false);
  ...
  if(phase==="signing")return <div className="onboarding employment-signing shadez-signing"><section><ShadezAvatar/><small>FOREMAN SHADEZ</small><h2>Excellent.</h2><p>Employment commenced.</p><span>ASSIGNMENT · OLD MINE</span></section></div>;
  ...
  return <div className="onboarding"><div><img className="onboarding-logo" src="/assets/brand/ore-whore-logo-primary.webp" alt="ORE WHORE — Property of the Department"/><p className="eyebrow">VOLUME I · CLASSIC → TBC → WOTLK</p><h1>DIG. CLANK. CRACK. <i>COLLECT.</i></h1><p>Fifteen ores. Fifteen minerals. 225 reasons not to stop.</p><button onClick={()=>agreementRequired?setPhase("management"):onDone(false)}>{agreementRequired?"REPORT TO MANAGEMENT":"RETURN TO MINING"} <span>→</span></button><small>{agreementRequired?"One vacancy. No meaningful qualifications required.":"Click the rock or press Space. That is genuinely it."}</small></div></div>;
}
