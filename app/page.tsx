...
type Save = { digs: number; emptyDigs:number; strikes: number; distance: number; combos: Record<string, number>; ores: Record<string, number>; oreResources:Record<string,number>;mineralResources:Record<string,number>;processedResources:Record<string,number>;ownedTools:string[];equippedTool:string;toolTier:number;toolSkinId:string; minerals: Record<string, number>; first: Record<string, number>; achievements: string[]; streak: number; longestStreak:number;newStreak:number;longestNewStreak:number; dust: number; dustEarned:number;dustSpent:number; biome: Biome; unlockedBiomes:Biome[]; completedBiomes:Biome[]; milestones: Record<string, number>; lastDigAt: number; schema: number; settings:Settings;unlocks:string[];equipped:string;huntTarget:string|null;huntCounts:Record<string,number>;huntStartedAtDig:number;longestHunt:number;trueArtifacts:Record<string,number>;trueFirst:Record<string,number>;activeTrueEncounter:ActiveTrueEncounter|null;misses:number;veinOre:string|null;veinDigsRemaining:number;milestoneDigs:Record<string,number> };
...
const migrate=(old:any):Save=>{
  ...
  const provisional={...blank,...old,combos,ores:oreCounts,oreResources,mineralResources,processedResources,ownedTools,equippedTool,toolTier,toolSkinId,first,minerals:mineralCounts,activeTrueEncounter:validEncounter,settings:{...defaultSettings,...old.settings},schema:14} as Save;
...
export default function Home() {
  ...
  const completeTrueEncounter=()=>{if(!pendingTrue)return;const {artifact,trigger}=pendingTrue,trueChance=equippedTrueArtifactChance(equippedMiningTool(save));playImpact("crack");setSave(s=>{const digNumber=s.digs+1,isFirst=!s.trueArtifacts[artifact.id],veinDigsRemaining=Math.max(0,s.veinDigsRemaining-1),veinExpired=s.veinDigsRemaining>0&&veinDigsRemaining===0;if(veinExpired)emitGameplayEvent("VEIN_EXPIRED",{ore:s.veinOre});setTrueFind({artifact,digNumber});return {...s,digs:digNumber,trueArtifacts:{...s.trueArtifacts,[artifact.id]:(s.trueArtifacts[artifact.id]||0)+1},trueFirst:isFirst?{...s.trueFirst,[artifact.id]:digNumber}:s.trueFirst,activeTrueEncounter:null,lastDigAt:Date.now(),veinDigsRemaining,veinOre:veinExpired?null:s.veinOre}});sessionDigs.current++;setSessionDigsCount(c=>c+1);setPendingTrue(null);track("true_artifact_found",{artifact_id:artifact.id,attempt:save.digs+1,biome:save.biome,trigger,tool_id:save.equippedTool,true_artifact_chance:trueChance,health_multiplier:20});emitGameplayEvent("TRUE_ARTIFACT_FOUND",{artifact_id:artifact.id,trigger,tool_id:save.equippedTool,health_multiplier:20});};
  ...
  const strike = (point?:{x:number;y:number}) => {
    ...
    if (stage === "tunnel") {
      ...
      if(rng.current()<trueChance){beginTrueEncounter(pickTrue(rng.current),"empty");return;}
    ...
    const trueChance=equippedTrueArtifactChance(equippedMiningTool(save));
    if(rng.current()<trueChance){beginTrueEncounter(pickTrue(rng.current),"ore");return;}
    const band = depthBand(maxHp);
