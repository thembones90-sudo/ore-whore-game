...
type Save = { digs: number; emptyDigs:number; strikes: number; distance: number; combos: Record<string, number>; ores: Record<string, number>; rawResources:Record<string,number>;processedMaterials:Record<string,number>;ownedTools:string[];equippedTool:string;toolSkinId:string; minerals: Record<string, number>; first: Record<string, number>; achievements: string[]; streak: number; longestStreak:number;newStreak:number;longestNewStreak:number; dust: number; dustEarned:number;dustSpent:number; biome: Biome; unlockedBiomes:Biome[]; completedBiomes:Biome[]; milestones: Record<string, number>; lastDigAt: number; schema: number; settings:Settings;unlocks:string[];equipped:string;huntTarget:string|null;huntCounts:Record<string,number>;huntStartedAtDig:number;longestHunt:number;trueArtifacts:Record<string,number>;trueFirst:Record<string,number>;activeTrueEncounter:ActiveTrueEncounter|null;misses:number;veinOre:string|null;veinDigsRemaining:number;milestoneDigs:Record<string,number> };
...
const blank: Save = { digs: 0, emptyDigs:0, strikes: 0, distance: 0, combos: {}, ores: {}, rawResources:{},processedMaterials:{},ownedTools:["rusty-pickaxe"],equippedTool:"rusty-pickaxe",toolSkinId:DEFAULT_TOOL_SKIN_ID, minerals: {}, first: {}, achievements: [], streak: 0,longestStreak:0,newStreak:0,longestNewStreak:0, dust: 0,dustEarned:0,dustSpent:0, biome: "old", unlockedBiomes:["old"], completedBiomes:[], milestones: {}, lastDigAt: 0, schema: 13,settings:defaultSettings,unlocks:[],equipped:"standard",huntTarget:null,huntCounts:{},huntStartedAtDig:0,longestHunt:0,trueArtifacts:{},trueFirst:{},activeTrueEncounter:null,misses:0,veinOre:null,veinDigsRemaining:0,milestoneDigs:{} };
...
function Forge({save,setSave}:{save:Save;setSave:React.Dispatch<React.SetStateAction<Save>>}){
  ...
  const runMetallurgy=(id:string)=>{const recipe=metallurgyRecipes.find(r=>r.id===id)!;setSave(s=>({...s,rawResources:spend(s.rawResources,recipe.inputs),processedMaterials:{...s.processedMaterials,[recipe.outputId]:(s.processedMaterials[recipe.outputId]||0)+recipe.outputQuantity}}));notify("success",`${recipe.operation} COMPLETE`,`+${recipe.outputQuantity} ${craftingName(recipe.outputId)} moved to the Alloy Stash.`)};
  const runForge=(id:string)=>{const recipe=forgeRecipes.find(r=>r.id===id)!,tool=forgedItems.find(t=>t.id===recipe.resultingItemId)!;setSave(s=>({...s,processedMaterials:spend(s.processedMaterials,recipe.inputs),ownedTools:[...s.ownedTools,recipe.resultingItemId],equippedTool:recipe.resultingItemId}));notify("success",`${tool.name} FORGED`,`${tool.technicalName} is now equipped. TRUE chance ${(equippedTrueArtifactChance(tool)*100).toFixed(2)}%.`)};
  const current=equippedMiningTool(save);
  const selectedSkin=toolSkin(save.toolSkinId);
  const RecipeCard=({id}:{id:string})=>{const r=metallurgyRecipes.find(x=>x.id===id)!,open=recipeUnlocked(save,r.unlock),ready=open&&canAfford(save.rawResources,r.inputs);const act=()=>{if(!open)return notify("error","MACHINE LOCKED",`Reach the ${r.unlock?.mine?.toUpperCase()} MINE to use this station.`);if(!ready)return notify("error","NOT ENOUGH ROCK",`Missing: ${missing(save.rawResources,r.inputs)}.`);runMetallurgy(r.id)};return <article className={`machine-recipe ${!open?"locked":""}`}><header><span>{r.operation}</span><strong>{r.name}</strong></header><div className="recipe-equation">{r.inputs.map(i=><b key={i.id}>{craftingName(i.id)} ×{i.quantity}</b>)}<i>→</i><em>{craftingName(r.outputId)} ×{r.outputQuantity}</em></div><button className={!ready?"unavailable":""} aria-disabled={!ready} onClick={act}>{open?(ready?r.operation:"INSUFFICIENT ORE"):`LOCKED · ${r.unlock?.mine?.toUpperCase()} MINE`}</button></article>};
  return <section className="forge-page"><div className="workshop-smoke" aria-hidden="true"/><div className="workshop-header"><aside className="goblin-corner" aria-hidden="true"><span className="goblin-head">⌁</span><p>PEON SMART.<br/>PEON BUILD.<br/>PEON DIG DEEP.<br/><b>ZUG ZUG!</b></p></aside><div className="workshop-sign"><span>PEON TECHNOLOGY</span><small>IF IT BONKS, IT WORKS.</small></div><div className="equipped-board equipped-loadout"><div className="loadout-technology"><span>TECHNOLOGY · GAMEPLAY</span><strong>{current.name}</strong><small>{current.technicalName}</small><ToolArt tool={current}/></div><div className="loadout-model"><span>MODEL · APPEARANCE</span><strong>{selectedSkin.name}</strong><small>{selectedSkin.animation.id.replace("-"," ").toUpperCase()} MOTION</small><img src={selectedSkin.artwork} alt=""/></div><footer><b>TRUE <em>{(equippedTrueArtifactChance(current)*100).toFixed(2)}%</em></b><b>ACTION <em>{current.actionDurationMs}ms</em></b><b>IMPACT <em>{current.damage.toFixed(2)}×</em></b></footer></div></div>
    <section className="stock-rack"><h3>RAW RESOURCE STOCK</h3><div className="material-ledger raw-ledger">{ores.map(o=><article key={o.id}><img src={oreAsset(o.id)} alt=""/><span>{o.name.replace(" Ore","")}</span><strong>{save.rawResources[o.id]||0}</strong></article>)}</div></section>
    ...
    <section className="alloy-stash"><h3>ALLOY STASH</h3><div className="material-ledger processed-ledger">{processedMaterials.map(m=><article key={m.id}><span>{m.name}<small>{m.description}</small></span><strong>{save.processedMaterials[m.id]||0}</strong></article>)}</div></section>
    <section className="skin-locker"><h3>PICKAXE RACK <small>MODEL ONLY · TECHNOLOGY STATS STAY EQUIPPED</small></h3><div className="skin-rack">{toolSkins.map(skin=><article key={skin.id} className={`${save.toolSkinId===skin.id?"selected":""} skin-card-${skin.silhouette}`}><div className="skin-preview"><img src={skin.artwork} alt=""/></div><span>COSMETIC MODEL</span><strong>{skin.name}</strong><p>{skin.flavor}</p>{skin.bark&&<blockquote>PEON: “{skin.bark}”</blockquote>}<button onClick={()=>{setSave(s=>({...s,toolSkinId:skin.id}));notify("success",`${skin.name} SELECTED`,`Appearance changed. ${current.name} technology and all gameplay stats remain equipped.`)}}>{save.toolSkinId===skin.id?"ON THE RACK":"USE THIS MODEL"}</button></article>)}</div></section>
    <section className="arsenal"><h3>FORGED MINING TOOLS <small>BIGGER = BETTER</small></h3><div className="tool-progression">{forgedItems.map(tool=>{const owned=save.ownedTools.includes(tool.id),recipe=forgeRecipes.find(r=>r.resultingItemId===tool.id),open=!recipe||recipeUnlocked(save,recipe.unlock),ready=!!recipe&&open&&canAfford(save.processedMaterials,recipe.inputs);const act=()=>{if(tool.planned)return notify("info","BLUEPRINT INCOMPLETE",`${tool.name} exists only as a dangerous sketch. Recipe materials are not finalized.`);if(owned){setSave(s=>({...s,equippedTool:tool.id}));return notify("success",tool.name,save.equippedTool===tool.id?"Already equipped. Peon confirms bonking readiness.":`${tool.technicalName} equipped. TRUE chance ${(equippedTrueArtifactChance(tool)*100).toFixed(2)}%.`)}if(!open)return notify("error","BLUEPRINT LOCKED",`Forge the previous tool and reach the required mine first.`);if(!ready&&recipe)return notify("error","MATERIALS REQUIRED",`Missing: ${missing(save.processedMaterials,recipe.inputs)}.`);if(recipe)runForge(recipe.id)};return <article className={`${owned?"owned":""} ${save.equippedTool===tool.id?"equipped":""} ${tool.planned?"planned":""}`} key={tool.id}><span>TIER {tool.tier} · {tool.inputMode.toUpperCase()}</span><h3>{tool.name}</h3><h4>{tool.technicalName}</h4><ToolArt tool={tool}/><div className="tool-stats"><b>{(equippedTrueArtifactChance(tool)*100).toFixed(2)}%</b><b>{tool.actionDurationMs}ms</b><b>{tool.damage.toFixed(2)}×</b></div>{recipe?<div className="tool-cost">{recipe.inputs.map(i=><b key={i.id}>{craftingName(i.id)} ×{i.quantity}</b>)}</div>:<div className="tool-cost"><b>{tool.tier===0?"STARTING TOOL":"???"}</b></div>}<button className={tool.planned||(!owned&&!ready)?"unavailable":""} aria-disabled={tool.planned||(!owned&&!ready)} onClick={act}>{tool.planned?"🔒 BLUEPRINT LOCKED":save.equippedTool===tool.id?"EQUIPPED":owned?"EQUIP":open?(ready?"CRAFT":"MATERIALS REQUIRED"):"🔒 BLUEPRINT LOCKED"}</button></article>})}</div></section>
