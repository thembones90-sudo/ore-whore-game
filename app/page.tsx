...
function Forge({save,setSave}:{save:Save;setSave:React.Dispatch<React.SetStateAction<Save>>}){
  ...
  const runMetallurgy=(id:string)=>setSave(s=>{const recipe=metallurgyRecipes.find(r=>r.id===id);if(!recipe||!recipeUnlocked(s,recipe.unlock)||!canAfford(s.rawResources,recipe.inputs))return s;setNotice(`${recipe.name.toUpperCase()} · +${recipe.outputQuantity} ${craftingName(recipe.outputId)}`);return {...s,rawResources:spend(s.rawResources,recipe.inputs),processedMaterials:{...s.processedMaterials,[recipe.outputId]:(s.processedMaterials[recipe.outputId]||0)+recipe.outputQuantity}}});
  const runForge=(id:string)=>setSave(s=>{const recipe=forgeRecipes.find(r=>r.id===id);if(!recipe||s.ownedTools.includes(recipe.resultingItemId)||!recipeUnlocked(s,recipe.unlock)||!canAfford(s.processedMaterials,recipe.inputs))return s;const tool=forgedItems.find(t=>t.id===recipe.resultingItemId)!;setNotice(`${tool.name} FORGED · AUTOMATICALLY EQUIPPED`);return {...s,processedMaterials:spend(s.processedMaterials,recipe.inputs),ownedTools:[...s.ownedTools,recipe.resultingItemId],equippedTool:recipe.resultingItemId}});
  const current=equippedMiningTool(save);
  const RecipeCard=({id}:{id:string})=>{const r=metallurgyRecipes.find(x=>x.id===id)!,open=recipeUnlocked(save,r.unlock),ready=open&&canAfford(save.rawResources,r.inputs);return <article className={`machine-recipe ${!open?"locked":""}`}><header><span>{r.operation}</span><strong>{r.name}</strong></header><div className="recipe-equation">{r.inputs.map(i=><b key={i.id}>{craftingName(i.id)} ×{i.quantity}</b>)}<i>→</i><em>{craftingName(r.outputId)} ×{r.outputQuantity}</em></div><button disabled={!ready} onClick={()=>runMetallurgy(r.id)}>{open?(ready?r.operation:"INSUFFICIENT ORE"):`LOCKED · ${r.unlock?.mine?.toUpperCase()} MINE`}</button></article>};
