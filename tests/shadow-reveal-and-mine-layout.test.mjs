...
test("Shadow uses the canonical reveal copy and dedicated silhouette beat",()=>{
  assert.match(artifacts,/id:"shadow"[\s\S]*announcement:"ANOMALOUS OBJECT DETECTED"[\s\S]*lore:"He cannot see it\. He knows exactly where it is\."[\s\S]*peonBark:"Cat\?"[\s\S]*systemResponse:"WUUUUUUUUUU"/);
