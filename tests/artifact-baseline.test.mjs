...
test("no technology tier owns an Artifact probability",()=>{for(const tool of forgedItems)assert.equal("trueArtifactChance" in tool,false)});
test("MOUNTAIN FUCKER cannot increase Artifact probability",()=>{assert.ok(forgedItems.find(t=>t.name==="MOUNTAIN FUCKER"));assert.equal(artifactChanceForDig(null),.0005)});
test("changing equipped technology cannot alter ordinary roll",()=>{for(const _tool of forgedItems)assert.equal(artifactChanceForDig(null),.0005)});
...
test("modifier is consumed after success or failure using the same state transition",()=>{for(const _result of [true,false])assert.equal(artifactChanceForDig(markModifierRolled(modifierFor("deep"))),.0005)});
